...
{
    //-------------------------------------------------------------------------
    public class VRTRIXGloveTeleport : MonoBehaviour
    {
        ...
        public VRTRIXGlovePlayer player;
        private LineRenderer pointerLineRenderer;
        private GameObject teleportPointerObject;
        private Transform pointerStartTransform;
        private VRTRIXGloveGrab pointerHand = null;

        private VRTRIXGloveTeleportArc teleportArc = null;
        private bool visible = false;

        private VRTRIXTeleportMarkerBase[] teleportMarkers;
        private VRTRIXTeleportMarkerBase pointedAtTeleportMarker;
        private VRTRIXTeleportMarkerBase teleportingToMarker;
        ...
        private VRTRIXAllowTeleportWhileAttachedToHand allowTeleportWhileAttached = null;
        ...
        public static VRTRIXGloveTeleport instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = GameObject.FindObjectOfType<VRTRIXGloveTeleport>();
                }
        ...
        void Awake()
        {
            ...
            teleportArc = GetComponent<VRTRIXGloveTeleportArc>();
        ...
        void Start()
        {
            teleportMarkers = GameObject.FindObjectsOfType<VRTRIXTeleportMarkerBase>();
        ...
        private void CheckForSpawnPoint()
        {
            ...
            {
                VRTRIXGloveTelportPoint teleportPoint = teleportMarker as VRTRIXGloveTelportPoint;
        ...
        void Update()
        {
            VRTRIXGloveGrab oldPointerHand = pointerHand;
            VRTRIXGloveGrab newPointerHand = null;
            //Debug.Log(player);
            foreach (VRTRIXGloveGrab hand in player.hands)
            {
                //Debug.Log(hand);
                if (visible)
                {
        ...
        private void TryTeleportPlayer()
        {
            if (visible && !teleporting)
            {
                ...
                {
                    Debug.Log("pointedAtTeleportMarker： " + pointedAtTeleportMarker);
        ...
        private void InitiateTeleportFade()
        {
            ...
            if (teleportPoint != null && teleportPoint.teleportType == VRTRIXGloveTelportPoint.TeleportPointType.SwitchToNewScene)
            {
                currentFadeTime *= 3.0f;
                VRTRIXGloveTeleport.ChangeScene.Send(currentFadeTime);
        ...
        //-------------------------------------------------
        private void HighlightSelected(VRTRIXTeleportMarkerBase hitTeleportMarker)
        {
        ...
        //-------------------------------------------------
        private bool IsTeleportButtonDown(VRTRIXGloveGrab hand)
        {
            if (IsEligibleForTeleport(hand))
            {
                //if (hand.noSteamVRFallbackCamera != null)
