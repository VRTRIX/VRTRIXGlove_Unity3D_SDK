...
{
    //-------------------------------------------------------------------------
    public class VRTRIXChaperoneInfo : MonoBehaviour
    {
        ...
        public static VRTRIXEvents.Event Initialized = new VRTRIXEvents.Event();
        public static VRTRIXEvents.Action InitializedAction(UnityAction action) { return new VRTRIXEvents.ActionNoArgs(Initialized, action); }

        //-------------------------------------------------
        private static VRTRIXChaperoneInfo _instance;
        public static VRTRIXChaperoneInfo instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new GameObject("[ChaperoneInfo]").AddComponent<VRTRIXChaperoneInfo>();
        ...
        IEnumerator Start()
        {
            ...
            while (true)
            {
                ...
                if (chaperone.GetPlayAreaSize(ref px, ref pz))
                {
                    ...
                    VRTRIXChaperoneInfo.Initialized.Send();
